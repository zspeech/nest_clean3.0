...
class WhiteNoiseAugmentation(object):
    """
    Augmentation that adds white noise to audio batches.
    
    This augmentation adds white noise directly to the audio signal in the batch,
    similar to WhiteNoisePerturbation but operates at the batch level.
    
    Args:
        prob (float): Probability of applying white noise augmentation (default: 0.0)
        min_level (int): Minimum noise level in dB (default: -90)
        max_level (int): Maximum noise level in dB (default: -46)
    """
    ...
    def __init__(
        ...
        if not (0 <= self.prob <= 1):
            raise ValueError(f"prob must be in [0, 1], got: {self.prob}")
        if not (self.min_level <= self.max_level):
            raise ValueError(
                f"min_level must be no greater than max_level, got: min={self.min_level} and max={self.max_level}"
            )
    
    def __call__(self, batch: AudioNoiseBatch) -> AudioNoiseBatch:
        """
        Add white noise to audio batch.
        
        Args:
            batch: AudioNoiseBatch containing audio signals
            
        Returns:
            AudioNoiseBatch with white noise added to audio signals
        """
        audio_signal = batch.audio
        audio_lengths = batch.audio_len
        batch_size = audio_signal.size(0)
        ...
        noisy_audio = audio_signal.clone()
        ...
        for i in range(batch_size):
            ...
            # Generate white noise for the actual audio length
            audio_len = audio_lengths[i].item()
            white_noise = torch.randn(audio_len, device=audio_signal.device, dtype=audio_signal.dtype) * noise_level_linear
            
            # Add white noise to the audio signal
            noisy_audio[i, :audio_len] = noisy_audio[i, :audio_len] + white_noise
        
        # Update noise field to contain the white noise that was added
        noise = noisy_audio - audio_signal
        noise_len = batch.audio_len.clone()
        ...
        return AudioNoiseBatch(
            ...
            noisy_audio_len=batch.audio_len,
        )
