...
"""
Device utilities for moving data to devices.
"""
...
def move_data_to_device(inputs: Any, device: Union[str, torch.device], non_blocking: bool = True) -> Any:
    ...
    - torch.Tensor: moves to device
    - list, tuple, set: recursively moves all elements
    - dict: recursively moves all values
    - dataclass: recursively moves all fields
    - other types: returns as-is
    
    Args:
        inputs: Data to move (can be tensor, list, dict, dataclass, etc.)
        device: Target device (string or torch.device)
        non_blocking: Whether to use non-blocking transfer (default: True)
        
    Returns:
        Data moved to the target device (same structure as input)
    """
    if inputs is None:
        return None
