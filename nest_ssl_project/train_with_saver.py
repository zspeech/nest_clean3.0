...
def main(cfg):
    ...
    # STRATEGY: Monkey patch the ConvFeatureMaksingWrapper class directly
    try:
        from modules.ssl_modules.masking import ConvFeatureMaksingWrapper
        if is_global_rank_zero():
            logger.info("Monkey patching ConvFeatureMaksingWrapper.forward for deterministic masking...")
        ...
        if is_global_rank_zero():
            logger.warning("Could not import ConvFeatureMaksingWrapper for patching. Using fallback object replacement.")
        
        if hasattr(asr_model, 'mask_processor'):
            if is_global_rank_zero():
                logger.info("REPLACING mask_processor with DeterministicMasking...")
    ...
    if hasattr(asr_model, 'preprocessor'):
        logger.info("FORCING PREPROCESSOR TO EVAL MODE for deterministic output")
        ...
        if hasattr(asr_model.preprocessor, 'featurizer'):
            ...
            logger.info("  Featurizer also set to eval mode")
